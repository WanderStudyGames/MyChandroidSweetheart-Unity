...
public class SoxAtkJiggleBone : MonoBehaviour
{
    ...
    private Transform targetTransformRoot;
    ...
    private Vector3 previousFramePositionDelta;
    ...
    public void Initialize()
    {
        ...
        targetTransformRoot = new GameObject("SoxAtkJiggleboneTargetRoot_" + transform.name).transform;
        targetTransformRoot.parent = transform.parent;
        targetTransformRoot.hideFlags = HideFlags.HideInHierarchy;
        targetTransformRoot.localPosition = transform.localPosition;
        targetTransformRoot.localRotation = transform.localRotation;
        targetTransformRoot.localScale = transform.localScale;

        // m_target은 지글본의 최후 도착지점 역할
        targetTransform = new GameObject("SoxAtkJiggleboneTarget_" + transform.name).transform;
        targetTransform.parent = targetTransformRoot;
        ...
        _child1 = new GameObject("SoxAtkJiggleboneProxyLook_" + transform.name).transform;
        _child1.parent = targetTransformRoot;
        _child1.hideFlags = HideFlags.HideInHierarchy;
        _child1.localPosition = Vector3.zero;
        _child1.localRotation = Quaternion.identity;
        _child1.localScale = Vector3.one;

        // m_proxyAlign은, m_proxyLook의 자식으로서 각종 축 옵션이 반영되어서 90도 단위로 회전된 상태의 노드.
        // 최종 룩앳 본이 이것에 정렬됨
        _child2 = new GameObject("SoxAtkJiggleboneProxyAlign_" + transform.name).transform;
        _child2.parent = _child1;
        _child2.hideFlags = HideFlags.HideInHierarchy;
        _child2.localPosition = Vector3.zero;
        _child2.localRotation = Quaternion.identity;
        _child2.localScale = Vector3.one;

        position = targetTransform.position;
        previousFramePositionDelta = Vector3.zero;
    ...
    private void SetHead()
    {
        ...
        if (transform.parent == null)
            return;
        SoxAtkJiggleBone headJiggleBone = transform.parent.GetComponent<SoxAtkJiggleBone>();
    ...
    public void JiggleBoneUpdate()
    {
        ...
        targetTransformRoot.position = transform.position;
        targetTransformRoot.localScale = transform.localScale;
        #region animated/tentacle
        if (m_animated)
        {
            if (m_tentacle == null)
            {
            ...
            {
                targetTransformRoot.localRotation = m_tentacle.localRotation * transform.localRotation;
            }
        }
        else
        {
            if (m_tentacle != null)
            {
                targetTransformRoot.localRotation = m_tentacle.localRotation;
            }
        }
        #endregion
        ...
        if (m_colliders.Length > 0)
        {
            ...
            for (int i = 0; i < m_colliders.Length; i++)
            {
                ...
                if (collider != null)
                {
                    ...
                    Vector3 lookFromCollider = targetTransform.position - collider.transform.position;
                    float lookFromColliderLength = lookFromCollider.magnitude;
                    // 타겟을 바라보는 벡터의 길이가 충돌체의 반지름보다 작다면 (충돌 구 안에 포함된다면)
                    if (lookFromColliderLength < collider.m_sphereRadiusScaled)
                    {
                        targetTransformPos =
                            collider.transform.position +
                            (lookFromCollider.normalized * collider.m_sphereRadiusScaled); // m_lookWPos를 충돌 구체 표면 위치로 옮긴다.
                    }

                    // Friction, 포스벡터를 계산하기 전이라서 이전 프레임의 관성으로만 검사한다. 포스벡터 연산 후에 충돌 검사하려면 연산이 더 필요해서 생략
                    // Collider가 여러 개 있을 수 있어서 여기서는 충돌 했는지만 검사
                    if (collider.m_frictionInverse > 0)
                    {
                        if (Vector3.Distance(position, collider.transform.position) < collider.m_sphereRadiusScaled)
                        {
                            collide = true;
                            friction = collider.m_frictionInverse;  // 마찰력은 최종적으로 충돌 검출된 Collider의 것을 사용
                        }
                    }
                }
            }
        ...
        switch (m_simType)
        {
            ...
            case SimType.Simple:
                {
                    // 이번 이동에서 발생한 포스 벡터
                    Vector3 force = (targetTransformPos - position);
                    // 이전 관성을 더한 뒤 텐션 적용한 결과 벡터 (DT?? 프레임이 떨어져서 DT가 증가하면 포스가 강해져야하고 inercia는 줄어들어야한다)
                    positionDelta = (force * m_tensionProxy * Time.unscaledDeltaTime) + (previousFramePositionDelta) * (Mathf.Lerp(m_inercia, 0.0f, Time.unscaledDeltaTime));
                    // Gravity
                    positionDelta += m_gravity * Time.unscaledDeltaTime;

                    // 충돌했을 경우 마찰력 반영
                    if (collide)
                    {
                        positionDelta *= friction;
                        previousFramePositionDelta *= friction;
                    }
            ...
            case SimType.KeepDistance:
                {
                    ...
                    m_boneEndDragWPos = (position - transform.position).normalized * m_targetDistance + transform.position;
                    // 이번 이동에서 발생한 포스 벡터 (본 길이를 감안한 포스), 비스듬하게 바라보면서 본 길이때문에 살짝 짧아지는 벡터가 된다.
                    Vector3 force = (targetTransformPos - m_boneEndDragWPos);
                    // 이전 관성을 더한 뒤 텐션 적용한 결과 벡터 (DT?? 프레임이 떨어져서 DT가 증가하면 포스가 강해져야하고 inercia는 줄어들어야한다)
                    positionDelta = (force * m_tensionProxy * Time.unscaledDeltaTime) + previousFramePositionDelta * (Mathf.Lerp(m_inercia, 0.0f, Time.unscaledDeltaTime));
                    // Gravity
                    positionDelta += m_gravity * Time.unscaledDeltaTime;
                    ...
                    m_boneEndDragWPos += positionDelta;
                    break;
                }
                #endregion
        ...
        else
        {
            ...
            if (!m_upWorld && m_upNode != null)
            {
                m_upVector = m_upNode.position - transform.position;
            }
        }
        ...
        try
        {
            ...
            _child1.LookAt(position, m_upVector);
            transform.rotation = _child2.rotation;
        }
        catch { }

        previousFramePositionDelta = positionDelta;
    }
    ...
    public bool Clear()
    {
        if (m_ifHead)
        {
            for (int i = 0; i < m_tree.Length; i++)
            {
                m_tree[i].position = m_tree[i].targetTransform.position;
                m_tree[i].previousFramePositionDelta = Vector3.zero;
            }
            return true;
        }
        else
    ...
    private void OnDrawGizmos()
    {
        ...
        if (Application.isPlaying)
        {
            ...
            if (m_optShowGizmosAtPlaying)
            {
                ...
                Gizmos.DrawWireSphere(targetTransform.position, gizmoSize);

                Gizmos.color = Color.green;
                Gizmos.DrawLine(transform.position, position);
                Gizmos.DrawWireSphere(position, gizmoSize * 0.6f);
            }
        }
        else
        {
    ...
    public void SetTargetDistance()
    {
        ...
        {
            case Axis.X:
                targetTransform.localPosition = new Vector3(m_targetDistance * targetFlip, 0.0f, 0.0f);
                break;
            case Axis.Y:
                targetTransform.localPosition = new Vector3(0.0f, m_targetDistance * targetFlip, 0.0f);
                break;
            case Axis.Z:
                targetTransform.localPosition = new Vector3(0.0f, 0.0f, m_targetDistance * targetFlip);
                break;
        }
    }
    ...
    public void SetOptions()
    {
        ...
        switch (m_lookAxis)
        {
            ...
            case Axis.X:
                ...
                {
                    case Axis.Y:
                        if (!m_lookAxisFlip && !m_sourceUpAxisFlip) { alignLookPos = _child1.TransformPoint(new Vector3(-1, 0, 0)); alignUpVector = _child1.TransformPoint(new Vector3(0, 1, 0)) - _child1.position; } // up
                        if (!m_lookAxisFlip && m_sourceUpAxisFlip) { alignLookPos = _child1.TransformPoint(new Vector3(1, 0, 0)); alignUpVector = _child1.TransformPoint(new Vector3(0, -1, 0)) - _child1.position; } // -up
                        if (m_lookAxisFlip && !m_sourceUpAxisFlip) { alignLookPos = _child1.TransformPoint(new Vector3(1, 0, 0)); alignUpVector = _child1.TransformPoint(new Vector3(0, 1, 0)) - _child1.position; } // up
                        if (m_lookAxisFlip && m_sourceUpAxisFlip) { alignLookPos = _child1.TransformPoint(new Vector3(-1, 0, 0)); alignUpVector = _child1.TransformPoint(new Vector3(0, -1, 0)) - _child1.position; } // -up
                        break;
                    case Axis.Z:
                        if (!m_lookAxisFlip && !m_sourceUpAxisFlip) { alignLookPos = _child1.TransformPoint(new Vector3(0, 1, 0)); alignUpVector = _child1.TransformPoint(new Vector3(1, 0, 0)) - _child1.position; } // right
                        if (!m_lookAxisFlip && m_sourceUpAxisFlip) { alignLookPos = _child1.TransformPoint(new Vector3(0, -1, 0)); alignUpVector = _child1.TransformPoint(new Vector3(-1, 0, 0)) - _child1.position; } // -right
                        if (m_lookAxisFlip && !m_sourceUpAxisFlip) { alignLookPos = _child1.TransformPoint(new Vector3(0, 1, 0)); alignUpVector = _child1.TransformPoint(new Vector3(-1, 0, 0)) - _child1.position; } // -right
                        if (m_lookAxisFlip && m_sourceUpAxisFlip) { alignLookPos = _child1.TransformPoint(new Vector3(0, -1, 0)); alignUpVector = _child1.TransformPoint(new Vector3(1, 0, 0)) - _child1.position; } // right
                        break;
            ...
            case Axis.Y:
                ...
                {
                    case Axis.X:
                        if (!m_lookAxisFlip && !m_sourceUpAxisFlip) { alignLookPos = _child1.TransformPoint(new Vector3(1, 0, 0)); alignUpVector = _child1.TransformPoint(new Vector3(0, 0, 1)) - _child1.position; } // forward
                        if (!m_lookAxisFlip && m_sourceUpAxisFlip) { alignLookPos = _child1.TransformPoint(new Vector3(-1, 0, 0)); alignUpVector = _child1.TransformPoint(new Vector3(0, 0, 1)) - _child1.position; } // forward
                        if (m_lookAxisFlip && !m_sourceUpAxisFlip) { alignLookPos = _child1.TransformPoint(new Vector3(-1, 0, 0)); alignUpVector = -_child1.TransformPoint(new Vector3(0, 0, -1)) - _child1.position; } // -forward
                        if (m_lookAxisFlip && m_sourceUpAxisFlip) { alignLookPos = _child1.TransformPoint(new Vector3(1, 0, 0)); alignUpVector = -_child1.TransformPoint(new Vector3(0, 0, -1)) - _child1.position; } // -forward
                        break;
                    case Axis.Z:
                        if (!m_lookAxisFlip && !m_sourceUpAxisFlip) { alignLookPos = _child1.TransformPoint(new Vector3(0, 1, 0)); alignUpVector = _child1.TransformPoint(new Vector3(0, 0, 1)) - _child1.position; } // forward
                        if (!m_lookAxisFlip && m_sourceUpAxisFlip) { alignLookPos = _child1.TransformPoint(new Vector3(0, -1, 0)); alignUpVector = _child1.TransformPoint(new Vector3(0, 0, 1)) - _child1.position; } // forward
                        if (m_lookAxisFlip && !m_sourceUpAxisFlip) { alignLookPos = _child1.TransformPoint(new Vector3(0, 1, 0)); alignUpVector = -_child1.TransformPoint(new Vector3(0, 0, -1)) - _child1.position; } // -forward
                        if (m_lookAxisFlip && m_sourceUpAxisFlip) { alignLookPos = _child1.TransformPoint(new Vector3(0, -1, 0)); alignUpVector = -_child1.TransformPoint(new Vector3(0, 0, -1)) - _child1.position; } // -forward
                        break;
            ...
            case Axis.Z:
                ...
                {
                    case Axis.X:
                        if (!m_lookAxisFlip && !m_sourceUpAxisFlip) { alignLookPos = _child1.TransformPoint(new Vector3(0, 0, 1)); alignUpVector = -_child1.TransformPoint(new Vector3(-1, 0, 0)) - _child1.position; } // -right
                        if (!m_lookAxisFlip && m_sourceUpAxisFlip) { alignLookPos = _child1.TransformPoint(new Vector3(0, 0, 1)); alignUpVector = _child1.TransformPoint(new Vector3(1, 0, 0)) - _child1.position; } // right
                        if (m_lookAxisFlip && !m_sourceUpAxisFlip) { alignLookPos = _child1.TransformPoint(new Vector3(0, 0, -1)); alignUpVector = _child1.TransformPoint(new Vector3(1, 0, 0)) - _child1.position; } // right
                        if (m_lookAxisFlip && m_sourceUpAxisFlip) { alignLookPos = _child1.TransformPoint(new Vector3(0, 0, -1)); alignUpVector = -_child1.TransformPoint(new Vector3(-1, 0, 0)) - _child1.position; } // -right
                        break;
                    case Axis.Y:
                        if (!m_lookAxisFlip && !m_sourceUpAxisFlip) { alignLookPos = _child1.TransformPoint(new Vector3(0, 0, 1)); alignUpVector = _child1.TransformPoint(new Vector3(0, 1, 0)) - _child1.position; } // up
                        if (!m_lookAxisFlip && m_sourceUpAxisFlip) { alignLookPos = _child1.TransformPoint(new Vector3(0, 0, 1)); alignUpVector = -_child1.TransformPoint(new Vector3(0, -1, 0)) - _child1.position; } // -up
                        if (m_lookAxisFlip && !m_sourceUpAxisFlip) { alignLookPos = _child1.TransformPoint(new Vector3(0, 0, -1)); alignUpVector = _child1.TransformPoint(new Vector3(0, 1, 0)) - _child1.position; } // up
                        if (m_lookAxisFlip && m_sourceUpAxisFlip) { alignLookPos = _child1.TransformPoint(new Vector3(0, 0, -1)); alignUpVector = -_child1.TransformPoint(new Vector3(0, -1, 0)) - _child1.position; } // -up
                        break;
        ...
        _child2.LookAt(alignLookPos, alignUpVector);
    ...
    public void SetHiddenNodes()
    {
        ...
        {
            targetTransformRoot.hideFlags = HideFlags.None;
            targetTransform.hideFlags = HideFlags.None;
            _child1.hideFlags = HideFlags.None;
            _child2.hideFlags = HideFlags.None;
            if (m_tentacle != null)
            {
    ...
    private void OnDestroy()
    {
        ...
        if (targetTransformRoot != null)
            GameObject.DestroyImmediate(targetTransformRoot.gameObject);
        if (_child1 != null)
            GameObject.DestroyImmediate(_child1.gameObject);
        if (_child2 != null)
            GameObject.DestroyImmediate(_child2.gameObject);
        if (m_tentacle != null)
