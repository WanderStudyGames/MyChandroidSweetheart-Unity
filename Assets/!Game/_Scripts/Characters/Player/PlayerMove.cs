...
public class PlayerMove : PlayerComponent
{
    ...
    public Vector2 GetWalkVector(Vector2 ignore, float maxSpeed)
    {
        var fuckyou = maxSpeed * 2 / 7;
    ...
    private Vector2 GetWorldSpaceWalkVector(Vector2 ignore)
    {
        float speedLimit = GetMaxSpeed();

        Vector2 worldSpaceWalkVector;

        if (_disableControls)
        {
            worldSpaceWalkVector = Vector2.zero;
            _walkVector = Vector2.zero;
        }
        else
        {
            _walkVector = GetWalkVector(_walkVector, speedLimit);
            worldSpaceWalkVector = transform.Vector2ToWorldSpace(_walkVector);
        }

        //if (IsGrounded)
        //{
        //_walkVector = _characterControllerMove.ApplyFriction(_walkVector);
        //}
        //ignore += worldSpaceWalkVector;
        return worldSpaceWalkVector;
    }
    ...
    private void OnEnable()
    {
        ...
        _characterControllerMove.HorizontalMovementFuncs.AddUnique(new(GetWorldSpaceWalkVector, GetType().Name));
    }
