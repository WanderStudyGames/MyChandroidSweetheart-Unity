...
public class Sprocket : PlayerComponent
{
    ...
    private void Start()
    {
        _defaultMaxLaunchForce = sprocketProfile.maxLaunchForce;
        CheckJumpItem();
    }
    ...
    private void SprocketLaunch()
    {
        ...
        if (canSprocket && castHit.collider != null)
        {
            ...
            else if (hasJumpItem)
            {
                ...
                if (canLaunch)
                {
                    var impulse = launchMultiplier * sprocketProfile.maxLaunchForce * sprocketProfile.PressureProfile.Evaluate(pressureDistance) * -_camera.transform.forward;
                    if (_jumpCombo) impulse = Vector3.Scale(impulse, new(4, 1.3f, 4));
                    else
                    {
                        impulse.x *= 2f;
                        impulse.z *= 2f;
                    }
                    _characterControllerMove.AddVelocity(impulse, false);
                    //fx
                    if (magnitude > sprocketProfile.bigChargeCutoff)
                    {
    ...
    private float _defaultMaxLaunchForce;
    private bool _jumpCombo;
    public void OnJump()
    {
        StopAllCoroutines();
        StartCoroutine(Co_JumpBoost());

        IEnumerator Co_JumpBoost()
        {
            _jumpCombo = true;
