...
namespace UnityEditor.ProBuilder
{
    class DrawShapeTool : EditorTool
    {
        ...
        static readonly Color k_PreviewColor = new(.5f, .9f, 1f, .56f);

        //Shape's properties
        internal bool m_IsOnGrid;

        internal Bounds m_Bounds;
        static readonly Color k_BoundsColor = new(.2f, .4f, .8f, 1f);

        static readonly GUIContent k_ShapeTitle = new("Create Shape");

        [UserSetting]
        internal static Pref<int> s_ActiveShapeIndex = new("ShapeBuilder.ActiveShapeIndex", 0);
        public static Pref<bool> s_SettingsEnabled = new("ShapeComponent.SettingsEnabled", false, SettingsScope.Project);

        [UserSetting]
        internal static Pref<int> s_LastPivotLocation = new("ShapeBuilder.LastPivotLocation", (int)PivotLocation.FirstCorner);
        [UserSetting]
        internal static Pref<Vector3> s_LastPivotPosition = new("ShapeBuilder.LastPivotPosition", Vector3.zero);
        [UserSetting]
        internal static Pref<Vector3> s_LastSize = new("ShapeBuilder.LastSize", Vector3.one);
        [UserSetting]
        internal static Pref<Quaternion> s_LastRotation = new("ShapeBuilder.LastRotation", Quaternion.identity);
        ...
        internal ProBuilderShape currentShapeInOverlay
        {
            get
            {
                ...
                if (m_ProBuilderShape == null)
                {
                    ...
                    m_ProBuilderShape.gameObject.layer = LayerMask.NameToLayer("env");
